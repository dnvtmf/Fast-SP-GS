...
from utils import add_bool_option, add_path_option, add_cfg_option, add_dict_option
...
class CheckpointManager:
    ...
    def store(self, name: str, cls: object, attr: str = None):
        ...
        if hasattr(getattr(cls, attr), "load_state_dict"):  # value is not None and
            ...
            if isinstance(obj, nn.Module):
                value = utils.state_dict_strip_prefix_if_present(value, "module.")
            ...
            logging.info(f"==> Resume `{name}`={utils.show_shape(value)}")
        return
    ...
    def save(
            self, filename: str = '', epoch=-1, save_dir: Path = None, use_prefix=True, manage=True, force=False,
            **kwargs
    ):
        """ save a checkpoint
        Args:
            ...
            force: force save checkpoint
            kwargs: Another data want to save to checkpoint
        """
        if not (force or utils.check_interval(epoch, self.checkpoint_interval) or epoch in self.save_at):
            return
        data = kwargs
        for name, (cls, attr) in self._store_objects.items():
            ...
            if hasattr(value, "state_dict"):
                ...
                if isinstance(value, torch.nn.Module):
                    value = utils.state_dict_strip_prefix_if_present(value.state_dict(), "module.")
            data[name] = value

        if not force and epoch in self.save_at:
        ...
        if not (force or utils.check_interval(epoch, self.checkpoint_interval)):
            return
        if filename.endswith(self.suffix):
            filename = filename[:-len(self.suffix)]
        if not force and self.num_checkpoint_max > 1:
            filename = f'{filename}_{epoch}'
        if use_prefix and not filename.startswith(self.prefix):
            filename = self.prefix + filename[len(self.prefix):]
        save_path = (self._save_dir if save_dir is None else save_dir).joinpath(filename + self.suffix)
