...
from utils import add_bool_option, add_path_option, str2dict, add_cfg_option
...
def options(parser=None):
    ...
    add_cfg_option(parser, "--fp16-cfg", help="The configure for torch.amp.GradScaler")
...
def make(cfg: argparse.Namespace):
    ...
    logging.info("==> Config:")
    logging.info(utils.config.save(cfg))

    if not hasattr(cfg, "seed") or cfg.seed < 0:
        cfg.seed = int(time.time() * 100)
    cfg.seed = (cfg.seed + 0) % 2 ** 31  # let seed be different
