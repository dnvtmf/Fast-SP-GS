...
__all__ = ['to_tensor', 'tensor_to', 'convert_pth', 'to_np',
...
def to_np(x, dtype=None, **kwargs) -> Optional[np.ndarray]:
    if x is None:
        return None
...
def show_shape(*inputs) -> str:
    ...
    for x in inputs:
        if isinstance(x, Tensor):
            ...
            s = f"'{x}'"
        elif isinstance(x, object) and x.__class__.__name__ in ['SE3', 'SO3']:
            s = f"{x.__class__.__name__}{list(x.shape)}"
        else:
            s = str(x)
        string.append(s)
