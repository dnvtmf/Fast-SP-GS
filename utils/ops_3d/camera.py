...
def compute_camera_align(p_unalgined: Tensor, p_target: Tensor, return_aligned: bool = False):
    ...
    if isinstance(p_unalgined, Tensor):
        # subtract mean
        mu1 = p_unalgined.mean(dim=-2, keepdim=True)
        mu2 = p_target.mean(dim=-2, keepdim=True)
        X1c = p_unalgined - mu1
        X2c = p_target - mu2
        # scale
        s1 = X1c.norm(dim=(-1, -2), keepdim=True)  # (X1c ** 2).sum(dim=-1).mean().sqrt()
        s2 = X2c.norm(dim=(-1, -2), keepdim=True)  # (X2c ** 2).sum(dim=-1).mean().sqrt()
        # s1 = (X1c ** 2).sum(dim=-1).mean().sqrt()
        # s2 = (X2c ** 2).sum(dim=-1).mean().sqrt()
        X1cs = X1c / s1
        X2cs = X2c / s2
        U, S, Vh = torch.linalg.svd((X1cs.transpose(-1, -2).double() @ X2cs.double()))
        R = (U @ Vh).to(p_target.dtype)  # type:Tensor
        R[..., 2, :] *= torch.where(R.det() < 0, -1, 1)[..., None]
        scale = S.sum(dim=(-1), keepdim=True)
        if return_aligned:
            return X1cs * scale, X2cs
        scale = scale[..., 0].float() * (s2 / s1)[..., 0, 0]
        t = mu2[..., 0, :] - torch.einsum('...i,...ij->...j', mu1[..., 0, :] * scale[..., None], R)
        R = R.transpose(-1, -2)
    else:
...
def camera_translate_scale(Tv2w: Tensor = None, K: Tensor = None, translate=0., scale=1.0):
    ...
    if isinstance(translate, (list, tuple)):
        translate = (Tv2w if Tv2w is not None else K).new_tensor(translate)
    if isinstance(scale, (list, tuple)):
        scale = (Tv2w if Tv2w is not None else K).new_tensor(scale)
    if Tv2w is not None:
...
def test_algin():
    ...
    scale = torch.rand(1) + 1.  # [1., 2.]
    R = SO3.exp(torch.randn(3)).matrix()[:3, :3]
    t = torch.rand(3) * 2 - 1.  # [-1, 1]
    points_ = (points * scale) @ R.T + t
    s_, R_, t_ = compute_camera_align(points + torch.randn_like(points) * 1e-9, points_)
    print()
    print(scale, s_, scale - s_)
    print(R - R_)
    print(t, t_, t - t_)
