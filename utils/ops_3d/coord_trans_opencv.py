...
def perspective(fovy: Union[float, Tensor] = 0.7854, aspect=1.0, n=0.1, f=1000.0, device=None, size=None, z01=True):
    """OpenCV 透视投影矩阵
    ...
    Args:
        ...
        size: (W, H)
        z01: Is z in range [0, 1] after perspective project
...
def perspective2(size, focals=None, FoV=None, pp=None, near_far=(0.1, 1000.0), device=None, z01=True):
    """OpenCV 透视投影矩阵
    ...
    Args:
        ...
        pp (float | list | tuple | Tensor | None): position of center, shape: [...], [..., 1], [..., 2]
        near_far (list | tuple | Tensor): near. Defaults to (0.1, 1000.0).
        device: Defaults to None.
...
def point2pixel(
    ...
    size: Tuple[int, int] = None, eps=1e-5,
):
    """
    Args:
    ...
    if Tv2s is not None:
        ...
        mask = points[..., 2:3].abs() < eps
        pixel = points[..., :2] / torch.where(mask, torch.full_like(points[..., 2:3], eps), points[..., 2:3])
        return pixel, points[..., 2]
    else:
        ndc = apply(points, Tv2c)
        ndc = ndc[..., :3] / torch.where(ndc[..., 3:].abs() < eps, torch.full_like(ndc[..., 3:], eps), ndc[..., 3:])
        pixel = torch.zeros_like(ndc[..., :2])
        pixel[..., 0] = (1 + ndc[..., 0]) * 0.5 * size[0]  # - 0.5
        pixel[..., 1] = (1 + ndc[..., 1]) * 0.5 * size[1]  # - 0.5
        return pixel, points[..., 2]
