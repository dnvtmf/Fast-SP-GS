...
__all__ = ['SequenceBatchSampler', 'ShuffleBatchSampler', 'IterableBatchSampler', 'BATCH_SAMPLERS']

BATCH_SAMPLERS = Registry()


@BATCH_SAMPLERS.register('sequence')
class SequenceBatchSampler(Sampler):
    ...
    def __init__(self, data_source, batch_size, drop_last=False, pad_last=False, **kwargs):
        ...
        self.num_replicas = 1  # get_world_size()
...
@BATCH_SAMPLERS.register('shuffle')
class ShuffleBatchSampler(Sampler):
    ...
    def __iter__(self):
        indices = self._random_generator.permutation(self.total)
        for i in range(self.length):
...
@BATCH_SAMPLERS.register('iterable')
class IterableBatchSampler(Sampler):
    ...
    def __iter__(self):
        if self.num_split == 1 and self.length > 0:
