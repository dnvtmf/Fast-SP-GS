...
from utils.ops_3d.spherical_harmonics import RGB2SH, SH_to_RGB, SH2RGB
from fast_2d_gs.losses import LossDict
from fast_2d_gs.renderer.gaussian_render import compute_cov3D as ComputeCov3D, render
...
class GaussianSplatting(nn.Module):
    ...
    def __init__(
        ...
        self._features_rest = nn.Parameter(torch.empty(0, (self.max_sh_degree + 1) ** 2 - 1, 3))
        self._scaling = nn.Parameter(torch.empty(0, 3))
        self._rotation = nn.Parameter(torch.empty(0, 3 if use_so3 else 4))
        ...
        if self.use_so3:
            self.toR = ops_3d.rotation.lie_to_R
        else:
            self.toR = ops_3d.rotation.quaternion_to_R
        ...
        self._step = -1
        self.loss_funcs = LossDict(**utils.merge_dict(loss_cfg))
    ...
    def set_from_dataset(self, dataset: NERF_Base_Dataset):
        ...
        if hasattr(dataset, 'cameras_extent'):
            ...
            self.cameras_extent = ops_3d.get_center_and_diag(dataset.cameras.Tv2w[:, :3, 3])[1].item() * 1.1
        self.lr_spatial_scale = self.cameras_extent
        logging.info(f"set camera_radius to {self.cameras_extent} ")
        if self.use_official_gaussians_render:
            assert dataset.perspective_z01, f"dataset.perspective_z01 must be True"
    ...
    def forward(self, scaling_modifier=1, campos: Tensor = None, **kwargs):
        ...
        if self.convert_SHs_python and campos is not None:
            outputs['colors'] = SH_to_RGB(features, points, campos, self.active_sh_degree)
        else:
    ...
    def prepare_inputs(self, info, t=None, background: Tensor = None, scale_modifier=1.):
        ...
        for b in range(Tw2v.shape[0]):
            ...
            raster_settings = {
                'size': size[b],
                'focals' if FoV is None else 'FoV': focals[b] if FoV is None else FoV[b],
                'Tw2v': Tw2v[b],
                'Tv2c': Tv2c[b],
                'campos': campos[b],
                'sh_degree': sh_degree,
    ...
    def render(self, *args, info, background: Tensor = None, scale_modifier=1.0, **kwargs):
        ...
        for b, (raster_settings, campos, bg, t) in enumerate(inputs):
            ...
            outputs_b = self.gs_rasterizer(**net_out, **raster_settings)
            ...
            if b == 0:
                ...
                for k, v in outputs_b.items():
                    if v is not None:
                        outputs[k].append(v)
        return {k: torch.stack(v, dim=0) if k != 'viewspace_points' and isinstance(v[0], Tensor) else v
    ...
    def training_setup(self):
        ...
        self.max_radii2D.data = torch.zeros((num_points,), device=self._xyz.device)
    ...
    def change_optimizer(
        ...
        assert op in ['replace', 'prune', 'concat', 'reset']
        optimizable_tensors = {}
        mask = None
        if name is not None:
        ...
        for optimizer in optimizers:
            for group in optimizer.param_groups:
                ...
                if op == 'concat':
                    ...
                    stored_state["exp_avg"][mask] = 0
                    stored_state["exp_avg_sq"][mask] = 0
                optimizer.state[group['params'][0]] = stored_state
        return optimizable_tensors

    def reset_optimizer(self, optimizer: torch.optim.Optimizer):
    ...
    def densify_and_split(self, optimizer: torch.optim.Optimizer, grads, grad_threshold, scene_extent, N=2):
        ...
        rots = self.toR(self._rotation[selected_pts_mask]).repeat(N, 1, 1)
        new_params = {
    ...
    def densify_and_clone(self, optimizer: torch.optim.Optimizer, grads, grad_threshold, scene_extent):
        ...
        selected_pts_mask = torch.logical_and(
            selected_pts_mask, torch.le(torch.amax(self.get_scaling, dim=1), scene_extent)
        )
