/*
Modified based on fused_ssim: https://raw.githubusercontent.com/rahul-goel/fused-ssim/refs/heads/main/ssim.cu
*/
...
__device__ float get_pix_value(
    const float* img, const int b, const int c, const int y, const int x, const int CH, const int H, const int W) {
  if (x >= W || y >= H || x < 0 || y < 0) {
    return 0.0f;
  } else {
...
__device__ void load_into_shared(
    float pixels[SY][SSX], const float* inp, const int CH, const int H, const int W, const int i) {
...
__global__ void fusedssimCUDA(int H, int W, int CH, float C1, float C2, float* img1, float* img2, float* ssim_map,
  ...
  for (int i = 0; i < CH; ++i) {
    ...
    if (pix_x < W && pix_y < H) {
      ...
      if (dm_dmu1) {
        dm_dmu1[global_idx]       = ((mu2 * 2.0f * D) / (A * B) - (mu2 * 2.0f * C) / (A * B) -
                               (mu1 * 2.0f * C * D) / (A * A * B) + (mu1 * 2.0f * C * D) / (A * B * B));
...
__global__ void fusedssim_backwardCUDA(int H, int W, int CH, float C1, float C2, float* img1, float* img2,
    float* dL_dmap, float* dL_dimg1, float* dm_dmu1 = nullptr, float* dm_dsigma1_sq = nullptr,
...
std::tuple<torch::Tensor, torch::Tensor, torch::Tensor, torch::Tensor> fusedssim(
    float C1, float C2, torch::Tensor& img1, torch::Tensor& img2, bool train) {
  CHECK_CUDA(img1);
  CHECK_CUDA(img2);
  ...
  fusedssimCUDA<<<grid, block>>>(H, W, CH, C1, C2, img1.contiguous().data<float>(), img2.contiguous().data<float>(),
      target.contiguous().data<float>(), dm_dmu1.contiguous().data<float>(), dm_dsigma1_sq.contiguous().data<float>(),
      dm_dsigma12.contiguous().data<float>());

  return std::make_tuple(target, dm_dmu1, dm_dsigma1_sq, dm_dsigma12);
}

torch::Tensor fusedssim_backward(float C1, float C2, torch::Tensor& img1, torch::Tensor& img2, torch::Tensor& dL_dmap,
    torch::Tensor& dm_dmu1, torch::Tensor& dm_dsigma1_sq, torch::Tensor& dm_dsigma12) {
  //   const at::cuda::OptionalCUDAGuard device_guard(device_of(img1));
  int B  = img1.size(0);
  int CH = img1.size(1);
  int H  = img1.size(2);
  ...
  dim3 grid((W + BX - 1) / BX, (H + BY - 1) / BY, B);
  dim3 block(BX, BY, 1);
  fusedssim_backwardCUDA<<<grid, block>>>(H, W, CH, C1, C2, img1.contiguous().data<float>(),
      img2.contiguous().data<float>(), dL_dmap.contiguous().data<float>(), dL_dimg1.contiguous().data<float>(),
      dm_dmu1.contiguous().data<float>(), dm_dsigma1_sq.contiguous().data<float>(),
      dm_dsigma12.contiguous().data<float>());

  return dL_dimg1;
}

REGIST_PYTORCH_EXTENSION(loss_ssim, {
  m.def("fusedssim", &fusedssim, "fusedssim");
  m.def("fusedssim_backward", &fusedssim_backward, "fusedssim_backward");
})